'use strict';

/**
 * scraper.js — Puppeteer media scraper for Supabase SQL output
 * Reads URLs from links.txt, scrapes media metadata,
 * deduplicates by title, and saves to output.sql incrementally.
 *
 * Usage:
 * node scraper.js [--visible] [--delay=2000] [--timeout=30000]
 */

const puppeteer = require('puppeteer');
const fs        = require('fs');
const path      = require('path');
const readline  = require('readline');

// ─── CONFIG ────────────────────────────────────────────────────────────────────

const LINKS_FILE  = path.resolve(__dirname, 'links.txt');
const OUTPUT_FILE = path.resolve(__dirname, 'output.sql');

const args = process.argv.slice(2);

const HEADLESS = !args.includes('--visible');

const DELAY = (function () {
    const flag = args.find(a => a.startsWith('--delay='));
    return flag ? parseInt(flag.split('=')[1], 10) : 1500;
})();

const TIMEOUT = (function () {
    const flag = args.find(a => a.startsWith('--timeout='));
    return flag ? parseInt(flag.split('=')[1], 10) : 30000;
})();

// ─── SELECTORS ─────────────────────────────────────────────────────────────────

const SEL = {
    title:      'h2.text-white.fw-bold.mb-1',
    playButton: 'button.play-icon-button',
    videoSrc:   'source[src]',
    poster:     'img.poster-image', // الإبقاء على المحدد كما هو لجلب الصورة وتحويل مسمى الحقل لاحقاً
};

// ─── HELPERS ───────────────────────────────────────────────────────────────────

/**
 * دالة بسيطة لهروب النصوص لتفادي مشاكل الـ SQL Injection أثناء توليد الملف
 * @param {string} str 
 * @returns {string}
 */
function escapeSql(str) {
    if (!str) return '';
    return str.replace(/'/g, "''");
}

/**
 * كتابة سجل جديد مباشرة إلى ملف الـ SQL بصيغة استعلام INSERT مع عمود الـ category و thumbnail_url
 * @param {{ title: string, video_url: string, thumbnail_url: string, category: string }} record
 */
function appendSqlRecord(record) {
    const query = `INSERT INTO videos (title, video_url, thumbnail_url, category) VALUES ('${escapeSql(record.title)}', '${escapeSql(record.video_url)}', '${escapeSql(record.thumbnail_url)}', '${escapeSql(record.category)}');\n`;
    fs.appendFileSync(OUTPUT_FILE, query, 'utf8');
}

/**
 * قراءة ملف الـ SQL الحالي (إذا وجد) لبناء الـ Set الخاص بمنع التكرار بناءً على العنوان
 * @returns {Set<string>}
 */
function loadExistingTitles() {
    const titles = new Set();
    if (!fs.existsSync(OUTPUT_FILE)) return titles;

    const content = fs.readFileSync(OUTPUT_FILE, 'utf8');
    const regex = /VALUES\s*\(\s*'(.*?)'/g;
    let match;
    while ((match = regex.exec(content)) !== null) {
        const originalTitle = match[1].replace(/''/g, "'");
        titles.add(originalTitle.toLowerCase());
    }
    return titles;
}

/**
 * Read all non-empty, non-comment lines from links.txt.
 * @returns {Promise<string[]>}
 */
function readLinks() {
    return new Promise((resolve, reject) => {
        if (!fs.existsSync(LINKS_FILE)) {
            return reject(new Error(`links.txt not found at: ${LINKS_FILE}`));
        }

        const lines = [];
        const rl = readline.createInterface({
            input: fs.createReadStream(LINKS_FILE),
            crlfDelay: Infinity,
        });

        rl.on('line', line => {
            const trimmed = line.trim();
            if (trimmed && !trimmed.startsWith('#')) lines.push(trimmed);
        });

        rl.on('close', () => resolve(lines));
        rl.on('error', reject);
    });
}

/**
 * Convert a potentially relative URL to an absolute URL using the page's origin.
 * Passthrough if already absolute.
 * @param {string} url
 * @param {string} origin  — e.g. "https://example.com"
 * @returns {string}
 */
function toAbsolute(url, origin) {
    if (!url) return '';
    if (/^https?:\/\//i.test(url)) return url;
    if (url.startsWith('//')) return 'https:' + url;
    return origin.replace(/\/$/, '') + '/' + url.replace(/^\//, '');
}

/**
 * Sleep for N milliseconds.
 * @param {number} ms
 * @returns {Promise<void>}
 */
function sleep(ms) {
    return new Promise(resolve => setTimeout(resolve, ms));
}

// ─── CORE SCRAPE LOGIC ─────────────────────────────────────────────────────────

/**
 * Scrape a single URL.
 * @param {import('puppeteer').Browser} browser
 * @param {string} url
 * @returns {Promise<{ title: string, video_url: string, thumbnail_url: string }>}
 */
async function scrapePage(browser, url) {
    const page = await browser.newPage();

    await page.setRequestInterception(true);
    page.on('request', req => {
        const type = req.resourceType();
        if (type === 'font') {
            req.abort();
        } else {
            req.continue();
        }
    });

    try {
        await page.goto(url, { waitUntil: 'networkidle2', timeout: TIMEOUT });

        // ── Title ────────────────────────────────────────────────────────────────
        await page.waitForSelector(SEL.title, { timeout: TIMEOUT });
        const title = await page.$eval(SEL.title, el => el.textContent.trim());

        if (!title) throw new Error('Title element found but empty');

        // ── Thumbnail (تم جلبها عبر محدد البوستر الأصلي للموقع) ───────────────────
        const origin = new URL(url).origin;

        const thumbnail_url = await page.$eval(
            SEL.poster,
            (el, org) => {
                const src = el.getAttribute('src') || el.getAttribute('data-src') || '';
                if (!src) return '';
                if (/^https?:\/\//i.test(src)) return src;
                if (src.startsWith('//')) return 'https:' + src;
                return org + '/' + src.replace(/^\//, '');
            },
            origin
        ).catch(() => '');

        // ── Click play, then wait for <source src> ────────────────────────────────
        await page.waitForSelector(SEL.playButton, { timeout: TIMEOUT });
        const playBtn = await page.$(SEL.playButton);
        if (!playBtn) throw new Error('Play button not found');

        await playBtn.click();

        await page.waitForFunction(
            (sel) => {
                const el = document.querySelector(sel);
                return el && el.getAttribute('src') && el.getAttribute('src').trim() !== '';
            },
            { timeout: TIMEOUT },
            SEL.videoSrc
        );

        const rawVideoSrc = await page.$eval(SEL.videoSrc, el => el.getAttribute('src') || '');
        const video_url = toAbsolute(rawVideoSrc, origin);

        if (!video_url) throw new Error('Could not extract video URL');

        return { title, video_url, thumbnail_url };

    } finally {
        await page.close();
    }
}

// ─── MAIN ──────────────────────────────────────────────────────────────────────

async function main() {
    console.log('═══════════════════════════════════════════');
    console.log('  ZiroTube — Puppeteer SQL Media Scraper');
    console.log('═══════════════════════════════════════════');
    console.log(`  Links file : ${LINKS_FILE}`);
    console.log(`  Output file: ${OUTPUT_FILE}`);
    console.log(`  Headless   : ${HEADLESS}`);
    console.log(`  Delay      : ${DELAY}ms between requests`);
    console.log(`  Timeout    : ${TIMEOUT}ms`);
    console.log('───────────────────────────────────────────\n');

    let links;
    try {
        links = await readLinks();
    } catch (e) {
        console.error(`[ERROR] ${e.message}`);
        process.exit(1);
    }

    if (!links.length) {
        console.warn('[WARN] links.txt is empty. Nothing to do.');
        process.exit(0);
    }

    console.log(`[INFO] Found ${links.length} URL(s) to process.\n`);

    const existingTitles = loadExistingTitles();
    console.log(`[INFO] Loaded ${existingTitles.size} existing record(s) from output.sql.\n`);

    const browser = await puppeteer.launch({
        headless: HEADLESS ? 'new' : false,
        args: [
            '--no-sandbox',
            '--disable-setuid-sandbox',
            '--disable-dev-shm-usage',
            '--disable-gpu',
        ],
    });

    let succeeded = 0;
    let skipped   = 0;
    let failed    = 0;

    for (let i = 0; i < links.length; i++) {
        const url = links[i];
        const label = `[${i + 1}/${links.length}]`;

        console.log(`${label} → ${url}`);

        try {
            const data = await scrapePage(browser, url);

            // Deduplication check
            if (existingTitles.has(data.title.toLowerCase())) {
                console.log(`${label} ⤳  SKIP — title already exists: "${data.title}"\n`);
                skipped++;
                continue;
            }

            // بناء السجل بالمسميات والحقول الجديدة المطلوبة لـ Supabase
            const record = {
                title:         data.title,
                video_url:     data.video_url,
                thumbnail_url: data.thumbnail_url,
                category:      'kids_movies', // الحقل الثابت الجديد
            };

            // الحفظ اللحظي داخل ملف الـ SQL
            appendSqlRecord(record);
            existingTitles.add(record.title.toLowerCase());

            console.log(`${label} ✓  Saved to SQL: "${record.title}"`);
            console.log(`        video_url     : ${record.video_url}`);
            if (record.thumbnail_url) console.log(`        thumbnail_url : ${record.thumbnail_url}`);
            console.log(`        category      : ${record.category}`);
            console.log();

            succeeded++;

        } catch (err) {
            console.error(`${label} ✗  FAILED: ${err.message}\n`);
            failed++;
        }

        if (i < links.length - 1) await sleep(DELAY);
    }

    await browser.close();

    console.log('═══════════════════════════════════════════');
    console.log('  Done.');
    console.log(`  ✓ Saved (SQL) : ${succeeded}`);
    console.log(`  ⤳ Skipped     : ${skipped}`);
    console.log(`  ✗ Failed      : ${failed}`);
    console.log('═══════════════════════════════════════════');
}

main().catch(err => {
    console.error('[FATAL]', err);
    process.exit(1);
});